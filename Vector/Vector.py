...
class Vector(object):
    ...
    def normalize(self):
        ...
        norm = self.norm()
        normed = tuple( x / norm if norm != 0 else x for x in self )
    ...
    def vector_mul(self, vector):
        if not isinstance(vector, Vector):
            raise ValueError('The required another vector')
        if not len(self) == len(vector):
            raise ValueError('Length must be equal')
        self.values = tuple(a * b for a, b in zip(self, vector))
